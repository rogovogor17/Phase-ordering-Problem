...
std::unique_ptr<IAlgorithm> AlgorithmFactory::create(AlgorithmType type,
...
std::unique_ptr<IEvaluator> EvaluatorFactory::create(const SolverConfig& config) {
    ...
    return std::make_unique<LLVMEvaluator>(sourceIR, std::move(llvm),
                                           std::move(analyzer), config.measureRuntime);
}
